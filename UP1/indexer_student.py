...
class Index:
    ...
    def indexing(self, m, l):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        temp_word_storage_list = [
            word if word.isalpha() else word[:-1] for word in m.split()]
        temp_word_storage_list = list(set(temp_word_storage_list))
        for word in temp_word_storage_list:
    ...
    def search(self, term):
        ...
        for index in self.index[term]:
            temp_tuple = (index, self.msgs[index])
...
class PIndex(Index):
    ...
    def get_poem(self, p):
        ...
        # IMPLEMENTATION
        # ---- start your code ---- #
        roman_index = self.int2roman[p]
        line_index = self.index[roman_index][0]

        while self.msgs[line_index] != '' or self.msgs[line_index+1] != '' or self.msgs[line_index+2] != '':
            poem.append(self.msgs[line_index])
            line_index += 1

        for increment in range(3):
            poem.append(self.msgs[line_index+increment])
        # ---- end of your code --- #
        return poem
